...
func (u UserModel) Where(user *entities.User, fieldName, fieldValue string) error {
	...
	row, err := u.db.Query("select id, fullname, email, username, password from users where "+fieldName+" = ? limit 1", fieldValue)
...
func (u UserModel) Create(user entities.User) (int64, error) {
	...
	result, err := u.db.Exec("insert into users (fullname, email, username, password) values(?,?,?,?)",
